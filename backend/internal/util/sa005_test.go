...
func sa005Setup(t *testing.T) (*service.NotificationService, *service.TimelineService, *repository.ApplicationProjectRepository, *gorm.DB) {
	t.Helper()
	...
	ns := service.NewNotificationService(tlRepo, msgRepo, logger)
	ts := service.NewTimelineService(tlRepo, logger)
...
func TestSA005ReminderCancel(t *testing.T) {
	ns, ts, appRepo, _ := sa005Setup(t)
...
func TestSA005ScanCancelledNoMsg(t *testing.T) {
	ns, ts, appRepo, db := sa005Setup(t)
	ctx, cancel := context.WithCancel(context.Background())
	cancel()
	if _, err := ns.ScanDeadlinesContext(ctx, ts, appRepo, 7); err == nil { t.Fatalf("expected cancellation error") }
	var total int64
...
func TestSA005CancelHelper(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	cancel()
	if err := service.ReminderCancelError(ctx); err == nil { t.Fatalf("expected cancellation error from helper") }
}

func TestSA005TimelineCancel(t *testing.T) {
	db, err := gorm.Open(sqlite.Open(":memory:"), &gorm.Config{})
	if err != nil { t.Fatalf("open: %v", err) }
	if sqlDB, err := db.DB(); err == nil { sqlDB.SetMaxOpenConns(1) }
	...
	ctx, cancel := context.WithCancel(context.Background())
	cancel()
	if _, err := repo.ListDue(ctx, time.Now()); err == nil { t.Fatalf("expected cancellation error") }
}
